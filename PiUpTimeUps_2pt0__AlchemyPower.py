...
##jwc o signal.signal(signal.SIGINT, keyboardInterruptHandler)
...
def get_VoltageAndTemp_Status_Fn(config_In):
	global ch0_mult, ch1_mult, ch2_mult, ch3_mult, address
	...
	#
	#
	print ("*** DEBUG: Date & Time               Vin   Vout  Batt-V  Board Temperature")
	##jwc o while (True):
	# Read Channel 0 - Input Voltage - max 5.5V.
	Vin = ch0_mult*getreading(address,channel0)
	config_In.batteryUps_Volts_Input_V = Vin
	#	if (Vin < V_in_min):
	#		Vin = 0
	...
	config_In.batteryUps_Volts_Battery_V = Vbattery

	# Read Channel 2 - Output V
	Vout = ch1_mult*getreading(address, channel2)
	config_In.batteryUps_Volts_Output_V = Vout
	...
	##jwc O print ("%s %5.2f %5.2f %5.2f %8.2fC %6.2fF" % (time.ctime(), Vin, Vout, Vbattery, TempC, TempF)) 
	print ("*** DEBUG: %s %5.2f %5.2f %5.2f %8.2fC %6.2fF" % (time.ctime(), Vin, Vout, Vbattery, TempC, TempF)) 
